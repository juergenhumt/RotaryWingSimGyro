...
dDrgN=[0.0085,0.0085,0.0105,0.0105];
dDrg1=[-0.0216,-0.0216, -0.0586, -0.0286];     %  blade drag coefficient                           :=: dDrg1
dDrg2= [0.4,0.30,0.27,0.27];
...
for k=1:4
  for j=1:length(x)
    f(j)=dDrg2(k)*x(j)^2 + dDrg1(k)*x(j) + dDrgN(k);
  end
  dL(k)=sprintf('%6.4f %6.4f %6.4f',dDrg2(k),dDrg1(k),dDrgN(k));
  plot(x,f,cl(k))
  hold on
  grid
end
